...
contract JoinTest is Test {
    ...
    function test_Join_Strategy() public {
        ...
        IFolksyStrategy.ShareBalance memory bal =
            strategy.getUserShareBalance(strategyId, curator, COMPTROLLER, moonwell_cbBTC);

        IFolksyStrategy.AssetBalance memory bal1 = strategy.getUserAssetBalance(strategyId, curator, cbBTC);
    ...
    function _createStrategy() internal returns (bytes32 strategyId) {
        ...
        IFolksyStrategy.Step[] memory steps = new IFolksyStrategy.Step[](2);

        // Step 0 - Supply half your cbBTC on Moonwell
        address[] memory _assetsIn0 = new address[](1);
        _assetsIn0[0] = cbBTC;
        steps[0] = IFolksyStrategy.Step({
