...
contract Strategy is Ownable2Step {
  ...
  mapping(bytes32 => IFolksyStrategy.Strategy) strategies;

  // strategyId => stats
  mapping(bytes32 => IFolksyStrategy.StrategyStats) strategyStats;

  // strategyId => user => userStats
  mapping(bytes32 => mapping(address => IFolksyStrategy.UserStats)) userStats;
  ...
  function createStrategy(
    ...
    IFolksyStrategy.StrategyStats storage stats = strategyStats[strategyId];
  ...
  function updateUserStats(
    ...
    IFolksyStrategy.AssetBalance memory tempAssetBal =
      IFolksyStrategy.AssetBalance({ assets: _assets, amounts: _assetsAmount });
    ...
    if (_userStats.tokenBalances.length == stepIndex) {
      ...
      IFolksyStrategy.AssetBalance memory userTokenBalances = _userStats.tokenBalances[stepIndex];
      IFolksyStrategy.ShareBalance memory userShareBalances = _userStats.shareBalances[stepIndex];
  ...
  function updateStrategyStats(
    ...
    IFolksyStrategy.StrategyStats storage stats = strategyStats[_strategyId];
  ...
  function getStrategy(
    bytes32 _strategyId
  ) public view returns (IFolksyStrategy.Strategy memory) {
   ...
   * @param _user address of the user to get strategies for
   */
  function getUserStrategyStats(
    bytes32 _strategyId,
    address _user
  ) public view returns (IFolksyStrategy.UserStats memory) {
  ...
  function getUserAssetBalance(
    ...
    address[] memory _assets,
    uint256 _stepIndex
  ) public view returns (IFolksyStrategy.AssetBalance memory) {
    ...
    for (uint256 i = 0; i < stats.tokenBalances.length; i++) {
      ...
      if (keccak256(abi.encode(stats.tokenBalances[i].assets)) == keccak256(abi.encode(_assets)) && i == stepIndex) {
        return stats.tokenBalances[i];
      }
    }
    return IFolksyStrategy.AssetBalance(_assets, new uint256[](0));
  }
  ...
  function getUserShareBalance(
    ...
    address _shareToken,
    uint256 _stepIndex
  ) public view returns (IFolksyStrategy.ShareBalance memory) {
    IFolksyStrategy.UserStats memory stats = userStats[_strategyId][_user];
    ...
    return IFolksyStrategy.ShareBalance(_protocol, _shareToken, 0, new address[](0), new uint256[](0));
  }
  ...
  function _validateSteps(
    IFolksyStrategy.Step[] memory steps
