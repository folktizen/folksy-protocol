...
contract Engine is ERC4626 {
    // might change later
    IFolksyStrategy strategyModule;
    ...
    constructor(address _strategyModule) ERC4626(IERC20(address(this))) ERC20("FOLKSY", "FOLK") {
        strategyModule = IFolksyStrategy(_strategyModule);
    }

    function join(bytes32 _strategyId, uint256[] memory amounts) public {
        // Fetch the strategy
        IFolksyStrategy.Strategy memory _strategy = strategyModule.getStrategy(_strategyId);
        ...
        for (uint256 i; i < _strategy.steps.length; i++) {
            // Fetch step
            IFolksyStrategy.Step memory _step = _strategy.steps[i];
